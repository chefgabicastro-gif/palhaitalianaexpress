...
import { ArrowLeft, Play, CheckCircle, Clock, Star, Crown, Sparkles, Trophy, Lock, Flame, GraduationCap, TrendingUp, Quote } from 'lucide-react';
...
export default function Aulas() {
  ...
  const fetchProgress = async () => {
    ...
    try {
      ...
      const {
        data: profile
      } = await supabase.from('profiles').select('xp').eq('user_id', user.id).maybeSingle();
      if (profile) {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-6">
        ...
        {/* Stats Overview */}
        <motion.div className="card-glow p-6 rounded-2xl mb-8" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            return <motion.div key={lesson.id} initial={{
              ...
              delay: index * 0.1
            }} className={`group cursor-pointer rounded-2xl overflow-hidden transition-all duration-300 ${isCompleted ? 'ring-2 ring-success/50' : 'hover:ring-2 hover:ring-primary/50'}`} onClick={() => setSelectedLesson(lesson)}>
                  {/* Thumbnail */}
                  <div className="relative aspect-video overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      {isCompleted ? <div className="w-14 h-14 rounded-full bg-success/90 flex items-center justify-center">
                        ...
                        </div> : <motion.div className="w-14 h-14 rounded-full bg-primary/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" whileHover={{
                    scale: 1.1
                  }}>
      ...
      {/* Video Player Modal */}
      <VideoPlayerModal lesson={selectedLesson} isOpen={!!selectedLesson} onClose={() => setSelectedLesson(null)} isCompleted={selectedLesson ? completedLessons.has(selectedLesson.id) : false} onComplete={handleCompleteLesson} />
    </div>;
}